...
class TestDepartment extends TestCase
{
  ...
  public function testUpdateDepartmentSuccess(): void
  {
    Sanctum::actingAs(User::factory()->create(), ['*']);
    $dept = Department::factory()->create();
    $response = $this->patch('/api/departments/' . $dept->id, [
      'code' => fake()->numerify('dept-####'),
      'name' => fake()->sentence(random_int(2, 4)),
      'alias' => fake()->lexify('??'),
    ]);
    ...
    $response->assertStatus(200)
      ->assertJson(
        fn (AssertableJson $json) =>
        $json->has('status')->where('status', true)
          ->has('message')->where('message', 'Success update a department')
          ->has('data', 6)
      );;
  }
  ...
  public function testDeleteDepartmentSuccess(): void
  {
    Sanctum::actingAs(User::factory()->create(), ['*']);
    $dept = Department::factory()->create();
    $response = $this->delete('/api/departments/' . $dept->id);

    $response->assertStatus(200)
      ->assertJson(
        fn (AssertableJson $json) =>
        $json->has('status')->where('status', true)
          ->has('message')->where('message', 'Success delete a department')
